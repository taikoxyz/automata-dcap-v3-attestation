...
contract DcapTestUtils {
        ...
        returns (bool success, EnclaveIdStruct.EnclaveId memory enclaveId)
    {
            ...
            if (decodedKey.eq("enclaveIdentity")) {
                ...
                for (uint256 j = 0; j < children[i].size(); j++) {
                    ...
                    if (decodedKey.eq("tcbLevels")) {
                        ...
                        bool parsedSuccessfully = _parseQuoteIdentityTcbLevels(enclaveId, tcbLevels, tcbLevelsSize);
        ...
        success = flag.miscselectFound && flag.miscselectMaskFound && flag.attributesFound && flag.attributesMaskFound
            && flag.mrsignerFound && flag.isvprodidFound && flag.tcbLevelsFound;
    }
            ...
            for (uint256 k = 0; k < tcbLevels[j].size(); k++) {
                ...
                if (decodedKey.eq("tcb")) {
                    ...
                    if (decodedKey.eq("isvsvn")) {
                        enclaveId.tcbLevels[j].tcb.isvsvn = uint16(JSONParserLib.parseUint(isvsvn.value()));
                    } else {
                        return false;
                    }
            ...
            for (uint256 k = 0; k < tcbLevels[j].size(); k++) {
                ...
                if (decodedKey.eq("tcb")) {
                    ...
                    if (decodedValue.eq("UpToDate")) {
                        ...
                        tcbInfo.tcbLevels[j].status = TCBInfoStruct.TCBStatus.TCB_OUT_OF_DATE_CONFIGURATION_NEEDED;
                    } else if (decodedValue.eq("ConfigurationNeeded")) {
                        tcbInfo.tcbLevels[j].status = TCBInfoStruct.TCBStatus.TCB_CONFIGURATION_NEEDED;
                    } else if (decodedValue.eq("ConfigurationAndSWHardeningNeeded")) {
                        tcbInfo.tcbLevels[j].status = TCBInfoStruct.TCBStatus.TCB_CONFIGURATION_AND_SW_HARDENING_NEEDED;
                    } else if (decodedValue.eq("SWHardeningNeeded")) {
                        tcbInfo.tcbLevels[j].status = TCBInfoStruct.TCBStatus.TCB_SW_HARDENING_NEEDED;
                    } else if (decodedValue.eq("Revoked")) {
